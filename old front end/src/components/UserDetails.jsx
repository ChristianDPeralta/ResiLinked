...
function UserDetails() {
  ...
  const loadUserActivity = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/activity/users/${userId}/activity`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const loadUserJobs = async () => {
    try {
      setJobsLoading(true)
      const response = await fetch(`http://localhost:5000/api/jobs?postedBy=${userId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const loadUserRatings = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/ratings/${userId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
