...
function EmployerDashboard() {
  ...
  const loadDashboardStats = async () => {
    try {
      ...
      const [jobsResponse, applicationsResponse] = await Promise.all([
        ...
        fetch('http://localhost:5000/api/jobs/my-applications-received', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ])
  ...
  const loadApplications = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch('http://localhost:5000/api/jobs/my-applications-received', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
  ...
  const loadWorkers = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch('http://localhost:5000/api/users/workers', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
  ...
  const handleApplication = async (applicationId, action, jobId, userId) => {
    try {
      ...
      if (action === 'accept') {
        endpoint = `http://localhost:5000/api/jobs/${jobId}/assign`
        method = 'POST'
      } else if (action === 'reject') {
        endpoint = `http://localhost:5000/api/jobs/${jobId}/reject`
        method = 'POST'
      }
